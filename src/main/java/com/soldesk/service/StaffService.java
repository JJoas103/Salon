...
public class StaffService {
    @Autowired private SalonMapper salonMapper;
    @Autowired private StylistMapper stylistMapper;
    @Autowired private StylistScheduleMapper scheduleMapper;
    @Autowired private ResvMapper resvMapper;
    ...
    private StylistVO assertOwnsStylist(int stylistId, int ownerId){
        ...
        if (stylist == null) throw new IllegalArgumentException("존재하지 않는 디자이너입니다.");
    ...
    @Transactional
    public void registerStylist(int salonId, int ownerId, StylistVO stylist, MultipartFile imageFile) throws IOException {
        assertOwnsSalon(salonId, ownerId);
    ...
    public List<Map<String, Object>> getScheduleGroups(int stylistId, int ownerId){
        ...
        while (i < schedules.size()) {
            ...
            while (j + 1 < schedules.size()) {
                ...
                boolean consecutive = LocalDate.parse(cur.getDate()).plusDays(1).equals(LocalDate.parse(next.getDate()));
    ...
    private static final String[] WEEKDAY_NAMES = {"월", "화", "수", "목", "금", "토", "일"}; // DayOfWeek.getValue(): 월=1 ... 일=7
    ...
    // schedules의 stylistId는 신뢰하지 않고 경로변수로 받은 stylistId로 덮어쓴다 (다른 디자이너 앞으로 등록되는 것 방지).
    @Transactional
    public String registerSchedules(int ownerId, int stylistId, List<StylistScheduleVO> schedules){
        ...
        for (StylistScheduleVO schedule : schedules) {
            if (schedule.getDate() == null || schedule.getDate().trim().isEmpty()) continue;
    ...
    public void deleteSchedule(int scheduleId, int ownerId){
        ...
        if (schedule == null) throw new IllegalArgumentException("존재하지 않는 스케줄입니다.");
