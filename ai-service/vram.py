...
6800XT 16GB 실측 — 상담 모델을 적재해둔 채로 512 편집이 돌아감
(--offload-to-cpu 기준 전용 12.0GB/16GB, 상담 모델도 밀려나지 않음)
VRAM 만 보면 이 락은 필요 없음

락을 남겨둔 이유는 연산 유닛 쪽임 — 위 측정은 상담 모델이 적재만 되어 있었고
추론은 하지 않은 상태였음. 둘이 진짜 동시에 도는 경합은 재보지 않았음

sd-server 는 올린 가중치를 내리는 엔드포인트가 없어 이미지 쪽은 비켜줄 수 없음
VRAM 이 모자란 것으로 확인되면 SDCPP_UNLOAD_CHAT=1 로 상담 모델을 내려가며 씀
