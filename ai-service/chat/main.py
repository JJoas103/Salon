...
class McpChatService:
    ...
    # "여성컷(컷, 라움헤어)" 형태에서 마지막 괄호 안 항목이 매장명
    # (AiChatController.formatHistoryItem 이 붙임)
    @staticmethod
    ...
    # 이력 매장이 뭘 취급하는지 미리 붙임
    # "매장을 확인해라" 지시만으로는 첫 턴에서 건너뜀
    @staticmethod
    ...
    # 마지막 메시지를 그냥 쓰면 빈 답변이 나감
    # recursion_limit 에 걸리면 끝 메시지가 "도구를 부르려던 중"이라 content 가 비어 있음
    @staticmethod
    ...
    async def ask(
        ...
        # 카탈로그에 없는 정보는 LLM 까지 보내지 않음
        # 대화기록에도 안 남김 — 이후 턴의 맥락이 오염됨
        ...
        # 이력이 필요한 질문인데 이력이 없으면 근거가 없음
        # "라움헤어에 뭐 있어" 처럼 매장을 직접 댄 질문은 걸리지 않음
    ...
    def _trim_history(self, messages: list[BaseMessage]) -> list[BaseMessage]:
        ...
        # 10턴이면 뒤로 갈수록 느려지고 같은 말을 반복함. 상담은 3~4턴이면 끝남
        if len(human_message_indexes) <= 6:
            return messages
        return messages[human_message_indexes[-6]:]

    # FastAPI 종료 시 MCP 세션과 하위 프로세스 정리
