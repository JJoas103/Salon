"""질문 갈래를 LLM 이 아니라 코드가 정함

작은 모델에 맡기면 건너뛰거나 한쪽으로 쏠려서, 무엇을 물었는지는 규칙으로 정하고
LLM 에게는 그 갈래에 필요한 데이터만 쥐여줌
확실한 것만 잡고 애매하면 SERVICE_SEARCH 로 떨어뜨림

도메인 자체를 벗어난 질문은 규칙으로 못 적으므로 chat/gate.py 가 따로 봄
"""
...
# 인덱스에 없는 정보 — 답하려면 지어낼 수밖에 없으므로 LLM 까지 보내지 않음
# 예약/결제/운영은 각자 화면이 있으니 그쪽으로 안내함
OUT_OF_SCOPE_RULES: list[tuple[str, str]] = [
    ...
    # 취소/변경만 잡던 동안 "어떻게 예약해" 가 새어나가 카카오·네이버예약을 안내했음
    # "손상모 케어 예약하고 싶어" 는 검색으로 보내야 하므로 방법을 묻는 형태만 잡음
    (r"예약\s*(방법|절차|어떻게|어디서|어디에서)"
     r"|예약\s*(은|는|을|를)\s*(어떻게|어디서|어디에서)"
     r"|예약\s*하려면|예약\s*하는\s*(법|방법)"
     r"|(어떻게|어디서|어디에서)\s*\S{0,4}\s*예약", "예약 방법"),
    (r"쿠폰|포인트|적립|할인\s*코드", "쿠폰·포인트"),
    (r"탈모\s*(치료|약|병원)|두피\s*질환|피부염|알레르기|부작용", "의학적 판단이 필요한 내용"),
    (r"회원\s*가입|탈퇴|비밀번호|로그인", "계정"),
]

# 매장 자체를 고르려는 질문 — salons 인덱스를 봐야 함
# "어디가 제일 저렴해" 처럼 수식어가 끼므로 인접 매칭 대신 두 낱말의 공존으로 판단함
...
# 특정 매장을 콕 집어 묻는 질문
# "뭐가 있어" 처럼 조사가 끼므로 의문사와 서술어 사이에 여유를 둠
...
# 증상만 있는 발화("머리 상했어요")는 따로 가르지 않음
# 규칙으로 가르면 "손상모 케어 추천해줘" 같은 정상 검색까지 되묻기로 샘
...
# 예약 이력을 봐야만 답할 수 있는 질문
# SALON_MENU 안에서도 이 쪽만 이력이 없으면 답이 안 나옴
...
def no_history_answer() -> str:
    ...
    LLM 에 보내면 "대화 내용에 없습니다" 같은 답에 40초를 씀
    """
    return (
...
# "상담에서 다루지 않습니다" 로 시작하면 안 되는 것들
# 예약은 이 서비스가 직접 하는 일이라 떠넘기면 그 자체로 오답
FULL_ANSWERS: dict[str, str] = {
    "예약 방법": (
        "예약은 이 서비스 안에서 바로 하실 수 있습니다.\n"
        "왼쪽 메뉴의 헤어샵 검색/예약에서 매장을 고르면 예약 화면이 열리고, "
        "시술 > 디자이너 > 날짜·시간 순으로 선택하시면 예약이 완료됩니다.\n\n"
        "어떤 시술을 받을지 아직 정하지 않으셨다면 고민을 말씀해 주세요. "
        "시술과 매장을 함께 찾아드리겠습니다."
    ),
}
...
def out_of_scope_answer(reason: str) -> str:
    ...
    if reason in FULL_ANSWERS:
        return FULL_ANSWERS[reason]

    guide = {
        "주차 정보": "매장 상세 페이지에 주소와 연락처가 있으니 매장에 직접 확인해 주세요.",
        "영업시간": "매장 상세 페이지에서 영업시간을 확인하실 수 있습니다.",
        # 예약 내역은 마이페이지 하위가 아니라 사이드바 최상위
        "예약·결제": "왼쪽 메뉴의 예약 내역에서 확인하고 취소하실 수 있습니다.",
