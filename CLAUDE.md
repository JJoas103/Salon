...
- **Already have a `salu` DB? Run `mysql -u root -p salu < sql/migration_catchup.sql` instead** — it brings any older DB up to the current schema and is idempotent (safe to re-run). `schema.sql` and `migration_catchup.sql` must always produce the same final schema; when you change one, change the other. Individual `migration_*.sql` files are folded into both and archived under `sql/archive/` (see its README for the workflow) — do not run them.
...
**Domain model** (`sql/schema.sql`, 25 tables): `Users` (customer/owner/admin) own `Salons`, which have `Services`, `Stylists`, `Stylist_Schedules`, `Salon_Operating_Hours`, `SalonNotices`. `Reservations` tie a user+salon+stylist+service together and drive `Reviews` and `Payments` (1:1 with a reservation); a payment settles `original_amount` down to `amount` against `Coupons`/`User_Coupons` and points, whose ledger is `Point_Transactions` (with `Users.point_balance` as the cached total). Separately: community (`Posts`/`Comments` plus `post_likes`, `post_reports`/`comment_reports`, `user_sanctions`), 1:1 `Chats`/`Messages` scoped per salon, `Wishlists`, `OwnerRequests` (customer→owner upgrade), and admin-run `Advertisements`.

- **Soft delete is the rule wherever a row has dependents** — `Users.deleted_at`, `Salons.closed_at`. Eight tables reference `salon_id` without `ON DELETE CASCADE`, so a hard delete just raises an FK error. Filter on these columns rather than adding delete statements.
- `Promotions` is the one table with no mapper and no screen — it's scaffolding that only `Coupons.promotion_id` references. Everything else in the schema is wired end-to-end (20 mapper XMLs).
